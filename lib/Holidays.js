...
function cachePath(cacheHomeDir, year) {
  return String(cacheHomeDir) + "/" + CACHE_FILE_PREFIX + Number(year) + ".json"
}

// curl writes the body straight to the cache file, which is what keeps the shell
// out of the writing business: QML has no file-write API, and the alternatives
// are a second process per refresh or no cache at all.
//
...
function fetchSpec(cacheHomeDir, year) {
  return {
    ...
    command: [CURL_BINARY,
      ...
      "--output", cachePath(cacheHomeDir, year),
      sourceUrl(year)]
  }
}
