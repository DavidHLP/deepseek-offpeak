...
// The three binaries the request names, by absolute path. Resolving any of them
...
// The most the request may deliver, enforced twice over. curl's --max-time
// bounds how long a transfer may take, not how much it may deliver, so a
// hostile or broken endpoint would otherwise decide how much memory the request
// costs: `head -c` closes the pipe at this many bytes whatever curl does, and
// the same bound is applied to the text before it is parsed. A balance document
// is a few hundred bytes, so this is generous and still small.
...
// line early.
//
// `head -c` is the producer-side byte cap, and `pipefail` is why it does not
// hide anything: without it the pipeline would report head's status and a curl
// that never connected would look like a success with an empty body.
var BALANCE_REQUEST_SCRIPT = [
  ...
  "printf 'header = \"Authorization: Bearer %s\"\\n' \"$key\" | "
    ...
    + " | " + HEAD_BINARY + " -c " + MAX_RESPONSE_BYTES
].join("\n")
