...
//   "resolve"        start a shell lookup
//   "wait"           a lookup is already in flight
//   "report-missing" settled with no key, and nothing asked us to look again
//
// `forceResolve` is what keeps this terminating. The resolver's own completion
// handler passes false: the lookup just finished, so re-arming it there would
// restart the resolver the moment it reports "no key", forever. Only a
// periodic or manual refresh passes true, which is why a key added to
// ~/.bashrc is still picked up without restarting the shell — once per poll,
// not once per completion.
function apiKeyAction(state, hasKey, resolverRunning, forceResolve) {
  if (hasKey) return "request"
  if (state === "resolving" || resolverRunning === true) return "wait"
  if (state === "ready" && forceResolve !== true) return "report-missing"
  return "resolve"
}
