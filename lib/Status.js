...
// exists to prevent.
//
// `apiKeySource` is "environment", "file", or "" — where the balance key was
// found, so a missing-key report says where it looked. Those two are the only
// places it is read from; see lib/Balance.js.
//
...
function text(state, balance) {
  ...
  // Only on a holiday, and immediately under the state it explains: a Chinese
  // public holiday is off-peak all day, so on one the state above is off-peak
  // whatever the hour is. Silent on every other day rather than printing an
  // empty field.
  var holiday = field(state, "holidayName", "")
  if (holiday !== "") lines.push("Holiday:       " + holiday + " \u2014 off-peak all day")
...
function keySourceLabel(source) {
  ...
  if (source === "file") return "the key file"
...
function object(state, balance) {
  ...
  return {
    ...
    // The Chinese public holiday covering now, by name, or "". Display text, not
    // an input: the schedule is already computed by the time this is written.
    holidayName: field(state, "holidayName", ""),
