...
// The binaries the plugin runs, by absolute path. Resolving any of them through
// PATH would mean a writable directory earlier in the search order chooses what
// actually runs.
...
// How much is read from the key file: one byte past the ceiling, so a file that
// is too large is recognizable as too large without being read whole. The read
// itself must be bounded — a file's size is not knowable without either reading
// it or asking someone, and the thing doing the reading here is `head -c`.
var KEY_FILE_READ_BYTES = KEY_FILE_MAX_BYTES + 1

// Deadline for that read. A local file returns immediately; a named pipe with
// no writer never does, and nothing else would stop this one.
var KEY_FILE_READ_TIMEOUT_MS = 2000
...
// What a bounded read of the key file means, so the widget and the CLI cannot
// disagree about it — and so the decision is testable without either of them.
//
// `text` is at most KEY_FILE_READ_BYTES and `readSucceeded` says whether the
// bounded read completed successfully. A failed or timed-out read is missing,
// even if it produced partial text; a successful read at the ceiling is a file
// too large to be a key file, whatever its first line looks like.
function keyFileResult(text, readSucceeded) {
  if (!readSucceeded) return { key: "", error: ERROR_MISSING_API_KEY }
  var value = String(text === null || text === undefined ? "" : text)
  if (value.length > KEY_FILE_MAX_BYTES) return { key: "", error: ERROR_INVALID_KEY_FILE }
  var parsed = parseKeyFile(value)
  if (parsed !== "") return { key: parsed, error: "" }
  // A file that is there and unreadable-as-a-key is a fixable mistake; no file
  // at all is simply no key.
  return { key: "", error: ERROR_INVALID_KEY_FILE }
}
...
// Currency values are also rendered by the plain-text status command. Reject
// terminal/control characters at the response boundary instead of letting
// display data become terminal instructions downstream.
var CURRENCY_CONTROL_RE = /[\u0000-\u001F\u007F-\u009F]/
...
// The runtime-specific adapters only need to execute this command and apply
// the returned outcome. Keeping the command, timeout, and output bound here
// prevents QML and Node from growing subtly different request policies.
function requestSpec() {
  return {
    command: [CURL_BINARY].concat(CURL_ARGUMENTS),
    timeoutMs: REQUEST_TIMEOUT_MS,
    maxBuffer: MAX_RESPONSE_BYTES
  }
}

// The key-file adapter is different in QML and Node, but its executable,
// bounded read, and path must be one contract. `path` is included for callers
// that need to explain or inspect which file was selected.
function keyReadSpec(configHome, home) {
  var path = keyFilePath(configHome, home)
  return {
    path: path,
    command: [HEAD_BINARY, "-c", String(KEY_FILE_READ_BYTES), "--", path],
    timeoutMs: KEY_FILE_READ_TIMEOUT_MS,
    maxBuffer: KEY_FILE_READ_BYTES
  }
}

// Node must preserve proxy/custom-CA variables when it supplies an explicit
// environment; QML passes null because its Process merges this small override
// into the inherited environment. Both adapters therefore use the same
// override policy and never put the key in a command.
function requestEnvironment(inherited, home, apiKey) {
  var environment = {}
  if (inherited && typeof inherited === "object") {
    for (var name in inherited) {
      if (Object.prototype.hasOwnProperty.call(inherited, name))
        environment[name] = inherited[name]
    }
  }
  if (home !== null && home !== undefined) environment.HOME = String(home)
  environment[KEY_ENVIRONMENT_VARIABLE] = String(apiKey === null || apiKey === undefined ? "" : apiKey)
  return environment
}
...
// Environment lookup is free and always wins. With no file outcome yet this
// returns `pending`, allowing an adapter to perform its bounded file read;
// after that read the same function decides the complete result for both
// runtimes.
function resolveKey(environmentValue, fileText, readSucceeded) {
  if (isUsableKey(environmentValue))
    return { status: "resolved", key: environmentValue, source: "environment", error: "" }

  if (fileText === undefined && readSucceeded === undefined)
    return { status: "pending", key: "", source: "", error: "" }

  var file = keyFileResult(fileText, readSucceeded === true)
  if (file.key !== "")
    return { status: "resolved", key: file.key, source: "file", error: "" }
  return { status: "error", key: "", source: "", error: file.error || ERROR_MISSING_API_KEY }
}
...
function parseBalanceEntry(raw) {
  ...
  if (CURRENCY_CONTROL_RE.test(raw.currency)) return null
...
// QML receives decoded text from SplitParser while Node's maxBuffer is byte
// based. Count the UTF-8 bytes here so both adapters enforce the same bound.
function utf8ByteLength(text) {
  var encoded
  try {
    encoded = encodeURIComponent(String(text === null || text === undefined ? "" : text))
  } catch (e) {
...
function fromResponse(exitCode, stdout) {
  ...
  if (utf8ByteLength(stdout) > MAX_RESPONSE_BYTES)
...
// Canonical state consumed by the QML service and the CLI status report. The
// optional balance is deliberately independent of schedule state, and the
// adapters supply any prior timestamp they intentionally retain on failure.
function idleState() {
  return { status: "idle", error: "", isAvailable: false, balances: [], updatedAtMs: 0 }
}

function loadingState(previous) {
  var prior = previous && typeof previous === "object" ? previous : idleState()
  return {
    status: "loading",
    error: "",
    isAvailable: prior.isAvailable === true,
    balances: Array.isArray(prior.balances) ? prior.balances : [],
    updatedAtMs: timestampOrZero(prior.updatedAtMs)
  }
}

function errorState(error, updatedAtMs) {
  return {
    status: "error",
    error: String(error || "error"),
    isAvailable: false,
    balances: [],
    updatedAtMs: timestampOrZero(updatedAtMs)
  }
}

function successState(result, updatedAtMs) {
  if (!result || result.ok !== true) return errorState(result && result.error, updatedAtMs)
  return {
    status: "ok",
    error: "",
...
// `failureUpdatedAtMs` preserves the QML service's existing meaning of a
// request failure: the last successful read time remains useful. A new CLI
// invocation passes zero, while key-resolution failures use errorState()
// directly and therefore reset it.
function stateFromResponse(exitCode, stdout, updatedAtMs, failureUpdatedAtMs, outputTooLarge) {
  if (outputTooLarge === true)
    return errorState(ERROR_INVALID_RESPONSE, failureUpdatedAtMs)
  if (exitCode === null || exitCode === undefined)
    return errorState(ERROR_NETWORK, failureUpdatedAtMs)

  var result = fromResponse(exitCode, stdout)
  if (!result.ok) return errorState(result.error, failureUpdatedAtMs)
  return successState(result, updatedAtMs)
}

function stateFromProcessError(error, failureUpdatedAtMs) {
  var code = error && error.code
  var stateError = code === "ENOBUFS" || code === "ERR_CHILD_PROCESS_STDIO_MAXBUFFER"
    ? ERROR_INVALID_RESPONSE : ERROR_NETWORK
  return errorState(stateError, failureUpdatedAtMs)
}
...
function describeState(state) {
  ...
  if (state.status === "ok")
    return describe({ ok: true, isAvailable: state.isAvailable, balances: state.balances })
  return describe({ ok: false, error: state.error })
}
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = {
    ...
    KEY_FILE_READ_BYTES: KEY_FILE_READ_BYTES,
    KEY_FILE_READ_TIMEOUT_MS: KEY_FILE_READ_TIMEOUT_MS,
    keyFilePath: keyFilePath,
    parseKeyFile: parseKeyFile,
    keyFileResult: keyFileResult,
    ...
    timestampOrZero: timestampOrZero,
    idleState: idleState,
    loadingState: loadingState,
    errorState: errorState,
    successState: successState,
    stateFromResponse: stateFromResponse,
    stateFromProcessError: stateFromProcessError,
    describe: describe,
